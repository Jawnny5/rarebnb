...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "Unable parse start date")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "Unable get parse end date")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "Unable to insert reservation into database!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
//ReservationSummary displays a page that presents a summary of information pertinent to reservation
...
func (m *Repository) ChooseRoom(w http.ResponseWriter, r *http.Request){
	// Previously implemented as past 6 lines
	...
	// Changed as follows, so more easily testable
...
//BookRoom takes URL params, builds a sessionable variable, and redirects to make reservation page.
...
//AdminShowReservation shows a singular reservation page
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request){
	//assuming no month or year is specified
	now := time.Now()
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request){
	...
	m.App.Session.Put(r.Context(), "flash", "Reservation verified & processed!")
	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-%s", src), http.StatusSeeOther)
}
...
//AdminPostReservationsCalendar handles posts from the reservations calendar.
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request){
	...
	for _, x := range rooms {
		//Grab block map from session, loop through whole mp & if we have an entry in the map 
		//that does not exist in our posted data, and if the restriction id is > 0, it should be removed.
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			//ok will be false if not present in map
			//what remains is placeholders for days w/out blocks.
